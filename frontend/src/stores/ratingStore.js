...
export const useRatingStore = defineStore('ratingStore', {
  ...
  actions: {
    async addNewRate(newRate) {
      try {
        const response = await axios.post('http://localhost:3000/api/v1/ratings', newRate);
    ...
    async fetchRatingsForBook(bookId) {
      try {
        const response = await axios.get(`http://localhost:3000/api/v1/ratings/book/${bookId}`);
